...
//		NSLog(@"Accelerometer not running on the device!");
    ...
    mgr.accelerometerUpdateInterval = accelerometerInterval;
    // 2.开始采样
    [self.mgr startAccelerometerUpdatesToQueue:[NSOperationQueue currentQueue] withHandler:^(CMAccelerometerData * _Nullable accelerometerData, NSError * _Nullable error) {
        ...
        if(isAccelerometerEnabled)
        {
            ...
            switch (deviceOrientation)
            {
                ...
                case UIDeviceOrientationLandscapeRight:
                    x = (isAccelerometerUpDownEnabled ? -accelerometerData.acceleration.z : 0.0f);
                    y = (isAccelerometerLeftRightEnabled ? -accelerometerData.acceleration.y : 0.0f);
